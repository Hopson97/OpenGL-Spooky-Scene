...
NK_API void nk_sfml_device_create(void)
{
    ...
    dev->attrib_col = glGetAttribLocation(dev->prog, "Color");
    {
        ...
        glVertexAttribPointer((GLuint)dev->attrib_col, 4, GL_UNSIGNED_BYTE, GL_TRUE, vs,
                              (void*)vc);
    }
...
NK_API void nk_sfml_render(enum nk_anti_aliasing AA, int max_vertex_buffer,
        ...
        elements = glMapBuffer(GL_ELEMENT_ARRAY_BUFFER, GL_WRITE_ONLY);
        {
            ...
            static const struct nk_draw_vertex_layout_element vertex_layout[] = {
                {NK_VERTEX_POSITION, NK_FORMAT_FLOAT,
                 NK_OFFSETOF(struct nk_sfml_vertex, position)},
...
static void nk_sfml_clipboard_paste(nk_handle usr, struct nk_text_edit* edit)
{
#if 0
...
static void nk_sfml_clipboard_copy(nk_handle usr, const char* text, int len)
{
#if 0
    char* str = 0;
    (void)usr;
...
NK_API int nk_sfml_handle_event(sf::Event* evt)
{
    ...
    if (evt->type == sf::Event::KeyReleased || evt->type == sf::Event::KeyPressed)
    {
        ...
        else if (key == sf::Keyboard::End)
        {
            ...
            nk_input_key(ctx, NK_KEY_TEXT_UNDO,
                         down && sf::Keyboard::isKeyPressed(sf::Keyboard::LControl));
    ...
    else if (evt->type == sf::Event::TouchMoved)
    {
        if (ctx->input.mouse.grabbed)
        {
            ...
            nk_input_motion(ctx, evt->touch.x, evt->touch.y);
        return 1;
    }
